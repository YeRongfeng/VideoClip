...
from utils import get_frame_range_limits
...
class VideoControlPanel:
    ...
    def _create_preview_controls(self):
        ...
        self.prev_frame_btn = tk.Button(
            playback_frame, text="上一帧", command=self._prev_frame, width=8
        )
        self.prev_frame_btn.pack(side=tk.LEFT, padx=2)

        self.play_pause_btn = tk.Button(
            playback_frame, text="播放", command=self._toggle_play, width=8
        )
        self.play_pause_btn.pack(side=tk.LEFT, padx=2)

        self.next_frame_btn = tk.Button(
            playback_frame, text="下一帧", command=self._next_frame, width=8
        )
        ...
        self.set_start_btn = tk.Button(
            playback_frame, text="设为起始帧", command=self._set_start_frame, width=10
        )
        self.set_start_btn.pack(side=tk.LEFT, padx=2)

        self.set_end_btn = tk.Button(
            playback_frame, text="设为结束帧", command=self._set_end_frame, width=10
        )
    ...
    def _create_trim_controls(self):
        ...
        self.trim_checkbox = tk.Checkbutton(
            trim_frame,
            text="启用时间裁切",
        ...
        # 开始帧控制
        tk.Label(trim_frame, text="开始帧:", bg=self.bg_color).pack(side=tk.LEFT, padx=(20, 5))
        self.start_frame_slider = tk.Scale(
            trim_frame,
            from_=0,
            to=1000,
            orient=tk.HORIZONTAL,
        ...
        # 结束帧控制
        tk.Label(trim_frame, text="结束帧:", bg=self.bg_color).pack(side=tk.LEFT, padx=(20, 5))
        self.end_frame_slider = tk.Scale(
            trim_frame,
            from_=1,
            to=1000,
            orient=tk.HORIZONTAL,
    ...
    def _create_time_info(self):
        ...
        self.time_info_label = tk.Label(time_info_frame, text="", bg=self.bg_color, fg="blue")
    ...
    def _create_crop_params(self):
        ...
        tk.Label(param_frame, text="裁切区域:", bg=self.bg_color).grid(row=0, column=0, sticky="w")
        self.pos_label = tk.Label(param_frame, text="X: 0, Y: 0", bg=self.bg_color)
        ...
        self.size_label = tk.Label(param_frame, text="宽: 0, 高: 0", bg=self.bg_color)
    ...
    def _create_action_buttons(self):
        ...
        self.preview_button = tk.Button(
            button_frame, text="预览裁切", command=self._preview_crop, state=tk.DISABLED, width=15
        )
        self.preview_button.pack(side=tk.LEFT, padx=10)

        self.process_button = tk.Button(
            button_frame, text="处理视频", command=self._process_video, state=tk.DISABLED, width=15
        )
...
class VideoCanvas:
    ...
    def show_frame(self, frame):
        ...
        self.canvas.create_image(
            offset_x + new_width // 2, offset_y + new_height // 2, image=tk_image, anchor=tk.CENTER
        )
    ...
    def show_preview(self, cropped_frame):
        ...
        self.canvas.create_image(
            canvas_width // 2, canvas_height // 2, image=tk_preview, anchor=tk.CENTER
        )
        self.preview_tk_image = tk_preview  # 防止垃圾回收

        # 添加返回按钮
        self.back_button = tk.Button(
            self.canvas, text="返回预览", command=self._back_to_preview, bg="#e0e0e0", fg="#000000"
        )
...
class StatusBar:
    ...
    def __init__(self, parent):
        ...
        self.status_bar = tk.Label(
            parent, text="就绪", bd=1, relief=tk.SUNKEN, anchor=tk.W, bg="#d0d0d0"
        )
        self.status_bar.pack(side=tk.BOTTOM, fill=tk.X)

        self.scale_info = tk.Label(
            parent, text="缩放比例: -", bd=1, relief=tk.SUNKEN, anchor=tk.W, bg="#d0d0d0"
        )
