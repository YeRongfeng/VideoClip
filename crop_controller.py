...
class CropController:
    ...
    def start_draw(self, event):
        ...
        # 确保点击在图像范围内
        if (event.x < self.canvas.display_offset[0] or 
            event.x > self.canvas.display_offset[0] + self.canvas.display_image.width or
            event.y < self.canvas.display_offset[1] or 
            event.y > self.canvas.display_offset[1] + self.canvas.display_image.height):
            return
    ...
    def stop_draw(self, event):
        ...
        if hasattr(self.canvas, 'current_frame') and self.canvas.current_frame is not None:
            ...
            # 重新绘制矩形
            self.canvas.draw_crop_rectangle(self.crop_x, self.crop_y, 
                                          self.crop_width, self.crop_height)
            
            # 通知裁切参数变化
            if self.on_crop_changed:
                self.on_crop_changed(self.crop_x, self.crop_y, 
                                    self.crop_width, self.crop_height)
